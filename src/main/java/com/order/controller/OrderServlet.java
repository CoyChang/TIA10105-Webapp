...
//		System.out.println(action);
		...
		if("getOne_For_Display".equals(action)){
			...
			OrderVO orderVO = orderSvc.getOneOrder(orderNumber);
			if(orderVO == null) {
				errorMsgs.add("查無資料");
			}
		...
		if("update".equals(action)) {
			...
			OrderService orderSvc = new OrderService();			
			orderVO = orderSvc.updateOrder(orderNumber, userNumber, orderStatus, establishmentTime, note, shippingTime, receiver, shippingAddress, deliveryFee, total);
		...
		if("insert".equals(action)) {
			...
			Integer orderStatus = 1;	//新增時預設為「1(待處理)」狀態
			Timestamp establishmentTime = new Timestamp(System.currentTimeMillis());	//訂單成立取得當下日期時間
			Timestamp shippingTime = null;	//出貨時間預設是空值
			...
			OrderService orderSvc = new OrderService();
			orderVO = orderSvc.addOrder(userNumber, orderStatus, establishmentTime, note, shippingTime, receiver, shippingAddress, deliveryFee, total);
		...
		if("getOneUserNumber_For_Display".equals(action)) {
			...
			//查詢某會員編號的訂單
			OrderService orderSvc = new OrderService();
			List<OrderVO> listUserNumberVO = orderSvc.getOrderByUserNumber(userNumber);
...
//	        }
		...
		if("getOneOrderStatus_For_Display".equals(action)) {
			...
			OrderService orderSvc = new OrderService();
			List<OrderVO> listOrderStatueVO = orderSvc.getOrderByOrderStatus(orderStatus);
